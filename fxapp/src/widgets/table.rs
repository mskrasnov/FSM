...
use crate::{
    fl,
    message::{KeyboardAndMouse, Message},
    widgets::link_button,
};
use iced::{
    Element, Length,
    widget::{button, table, text, text::IntoFragment},
};
        ...
        Some(val) if !val.to_string().is_empty() && !val.to_string().starts_with("http") => {
            button(text(val.to_string()))
                ...
                .on_press(Message::KeyboardAndMouse(
                    KeyboardAndMouse::CopyButtonPressed(val.to_string()),
                ))
                .into()
        }
        Some(val) if !val.to_string().is_empty() && val.to_string().starts_with("http") => {
...
pub fn hdr_name<'a, S>(s: S) -> text::Text<'a>
where
    S: IntoFragment<'a>,
{
